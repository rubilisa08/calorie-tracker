...
`foods.json`의 262개 목록에 없는 음식을 위한 수동 기록 경로. **PRD 3.2의 "사용자가 새 음식을 직접 등록하는 기능"(Won't Have)과는 다르다** — `foods.json`이라는 공용 데이터베이스에 영구 등록되는 게 아니라, 그날의 `logs`에 1회성으로 저장되는 기록일 뿐이며 검색 결과에는 나타나지 않는다.
